...
import { FileText, ArrowLeft, CheckCircle, Users, Calendar, BarChart3, Settings, MessageCircle, Pin, Upload, Mic, User, Bell, Plus, LayoutGrid, Crown, Edit, Trash2, Share2, Eye, EyeOff, Save, X, Play } from 'lucide-react';
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              ...
              <span className="uppercase hidden md:inline">ПРОЙТИ ОБУЧЕНИЕ</span>
              <span className="uppercase md:hidden">ОБУЧЕНИЕ</span>
            </button>
            <button
              onClick={onClose}
              className="flex items-center space-x-2 px-4 py-2 text-white hover:bg-white/20 rounded-lg transition-colors backdrop-blur-sm"
              ...
              <div className="flex flex-col md:flex-row gap-6 items-center">
                <div className="flex-1">
                  ...
                  <div className="mb-6">
                    <button
                      onClick={handleStartTraining}
                      className="flex items-center space-x-2 px-6 py-3 text-white rounded-xl font-medium uppercase transition-all hover:opacity-90"
                      style={{ backgroundColor: '#b6c2fc' }}
                    >
                      <Play className="w-5 h-5" />
                      <span>ПРОЙТИ ИНТЕРАКТИВНОЕ ОБУЧЕНИЕ</span>
                    </button>
                    <p className="text-sm text-gray-600 mt-2">
                      Пройдите пошаговое обучение для {currentUser?.role === 'admin' ? 'администраторов' : 'пользователей'}
                    </p>
                  </div>
